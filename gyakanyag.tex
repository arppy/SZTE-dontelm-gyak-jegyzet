...
\begin{tabular}{c||c|c|c|c|c}
 & $X_1$& $X_2$& $X_3$& $X_4$& $X_5$ \\
 \hline
 $A_1$&68  &98 & lepukkant & közepes & lakáson belül \\
 $A_2$&74  &56 & új& kiemelkedően jó& fedett \\
 $A_3$&138  &120 & újszerű & kifejezetten rossz& nyitott udvaron \\
 $A_4$&76  &56 & új& kiemelkedően jó& fedett \\
\end{tabular}
\end{center}
...
Példa: 50 millió forint alatti ingatlanokat kiemelkedően jó értékűnek tartjuk. A 70 m$^2$-hez képest legfeljebb 15 m$^2$-rel eltérő ingatlanokat kiemelkedően jó alapterületűnek tartjuk. ...
...
\item nem skálázott (nem ismert a globális range),
...
\begin{tabular}{c||c|c|c|c|c}
 & $X_1$& $X_2$& $X_3$& $X_4$& $X_5$ \\
 \hline
 $A_1$&68  &28 & 0.3 & 0.56 & 0.1 \\
 $A_2$&74  &14 & 0.9 & 0.97& 0.9 \\
 $A_3$&138  &50 & 0.6 & 0.12& 0.5 \\
 $A_4$&76  &14 & 0.9 & 0.97& 0.9 \\
...
\begin{tabular}{c||c|c|c|c|c}
 & $X_1$& $X_2$& $X_3$& $X_4$& $X_5$ \\
  &\small{$max=150,$}&\small{$max=70,$}	&		&		&	\\
 &\small{$min=20$}&\small{$min=0$}	&		&		& \\
 \hline
 $A_1$&0.63  &0.6 &  0.3 & 0.56 & 0.1 \\
 $A_2$&0.58  &0.8 & 0.9 & 0.97 & 0.9 \\
 $A_3$&0.09  &0.29 & 0.6 & 0.12 & 0.5 \\
 $A_4$&0.57  &0.8 & 0.9 & 0.97 & 0.9 \\
\end{tabular}
\end{center}
...
Pl: legfontosabb szempont a biciklitárolási lehetőség, második pedig a lakás ára. Ebben az esetben a legfontosabb cél szerint két alternatíva marad, de a lakás ára alapján már egyértelműen lehet dönteni a fennmaradó két alternatíva közül.

\subsection{Maximin módszer (pesszimista döntéshozó)}

Az értékek azonos skálán vannak. Minden alternatívához megkeresi a felvett leggyengébb értéket és azok közül kiválasztja a legjobbat.

A példában az utolsó alternatíva legrosszabb értéke 0.57, az $A1$-é 0.1, az $A2$-é 0.58, míg az $A3$-é 0.12, így ezeknek a maximuma 0.58, ezért az $A2$ alternatívát választjuk.
...
A példában az $A1$ alternatíva maximális értéke 0.63, az $A2$-é és az  $A_4$-é  0.97, míg az $A3$-é 0.61.

Mivel több alternatíva maradt ezért közülük az eddigi kritériumok letakarásával a következő maximális érték mentén próbálunk dönteni,
mindaddig, míg nem lesz egyértelmű legjobb alternatíva. Az ár szerint már egyértelmű döntést tudunk hozni: $A_2$.


\subsection{Súlyozásos módszer}

Minden kritériumhoz meghatározunk egy súlyt és elvárjuk, hogy a súlyok összege 1 legyen.
Majd a súlyozott összegek alapján a maximális értékű alternatívát választjuk.

Például legyen a döntéshozó akarata alapján az $X_1$-hez tartozó súly 0.5, míg az $X_2$, $X_3$, $X_4$-hez 0.1, illetve az $X_5$-höz 0.2.
A súlyok összege 1, tehát megfelelő súlyozást adott meg a döntéshozó.
Ezek alapján kiszámoljuk az egyes alternatívákhoz kapcsolódó súlyozott összegeket.
Az $A_1$ alternatíva esetében $0.5\cdot0.63+0.1\cdot0.6+0.1\cdot0.3+0.1\cdot0.56+0.2\cdot0.1$, ami 0.481.
Az $A_2$ esetében ugyanez 0.737,
az $A_3$ esetében 0.246, %0.5×0.09+0.1×0.29+0.1×0.6+0.1×0.12+0.2×0.5
míg $A_4$ esetében 0.732. %0.5×0.57+0.1×0.8+0.1×0.9+0.1×0.97+0.2×0.9
Mivel a maximális értéket az $A_2$ veszi fel, ezért ezt az alternatívát választjuk.
...
Felmerül a kérdés, hogy milyen az, ha az indifferencia osztályok halmaza nem megszámlálható?
Például, amennyiben tökéletesen osztható javak tartoznak az infifferencia osztályokba, melyeknek tovább osztásával újabb osztályok hozhatóak létre.

Minden adott $v$ létezéséhez.

\subsection{Értékelő függvény létezése}

Ha a $\succeq$ reláció gyenge rendezés az $A$ halmaz elemeire vonatkozóan, és az indifferencia osztályok halmaza megszámlálható
$\Rightarrow \exists v: A \rightarrow \mathbb{R} $, ahol $\forall(a,b) \in A$ teljesül, hogy
\begin{itemize}
...
\paragraph{Speciális eset: A n dimenziós vektortér. }
Tehát $\forall a \in A$ alternatíva $x_1, ... , x_n$ komponenssel (tulajdonság) jellemzett.
Ekkor $v: \mathbb{R}^n \rightarrow \mathbb{R}$ lesz.
Feltesszük továbbra is, hogy a $\succeq$ reláció egy gyenge rendezés az $A$ halmaz elemeire.
És ebben az esetben az alábbi két további feltétel teljesülése esetén garantált $v$ létezése:
\begin{itemize}
\item monotonitási feltétel:
$\forall a,b \in A$ alternatíva, amely $x_1, ... , x_n$ és $y_1, ... , y_n$ komponenssel jellemzett,
teljesül, hogy $x_i \geq y_i$ ($\forall i = 1, ... , n$) és legalább egy $j$ esetén pedig
$x_j \geq y_j$ $\Rightarrow a \succ b$.
Tehát ha $a$ kritériumonként mindenben legalább olyan jó,
mint $b$ és létezik legalább egy kritérium, amiben $a$ egyértelműen jobb, mint $b$,
akkor szigorú preferenciának kell fennállnia $a$ javára: $a \succ b$.
\item folytonossági feltétel: $\forall a,b,c \in A$, ahol $a \succ b \succ c \Rightarrow$  $\exists k \in (0,1): b \sim k \cdot a + (1-k) \cdot c$.
Erre azért van szükség, mert a valós függvényeknek folytonosnak kell lennie $\mathbb{R}^n$-en, ezért a kis változások az
alternatívákban nem vezethetnek hirtelen, ugrásszerű változáshoz a preferenciákban.
\end{itemize}
Továbbiakban ezt a speciális esetét vizsgáljuk.

\subsection{Értékelő függvények dekompozíciós alakjai}

\textbf{Def: } Preferencia struktúra \textbf{dekomponálható}, ha
$\exists v_1: X_1 \rightarrow \mathbb{R}$, ..., $v_n: X_n \rightarrow \mathbb{R}$ és $\exists v: A \rightarrow \mathbb{R}$ függvények, hogy
$\forall a, b \in A: a \succeq  b \Leftrightarrow $
\begin{equation}
v(x_1, .., x_n)=v[v_1(x_1), ..., v_n(x_n)] \geq v(y_1, .., y_n)=v[v_1(y_1), ..., v_n(y_n)]
\end{equation}
...
Tegyük fel, hogy valaki nyáron a fagyit, télen a sütit részesíti előnyben akkor:
(fagyi, nyár) $\succeq$ (süti, nyár) $\Rightarrow$ nem teljesül, hogy (fagyi, tél) $\succeq$ (süti, tél),
ugyanis (süti, tél) $\succeq$ (fagyi, tél).
Ekkor azt mondjuk, hogy az édesség preferenciafüggő az évszaktól.
\textbf{De ez fordítva nem feltétlenül mondható ki.} NEM SZIMMETRIKUS.
Pl: (nyár, fagyi) $\succeq$ (tél, fagyi), és (nyár, süti) $\succeq$ (tél, süti).
Tehát az évszak preferenciafüggetlen az édességtől.
...
Ebben az esetben viszont feltesszük azt, hogy függetlenül az évszaktól és az édesség típusától a csokisat preferáljuk a vaníliással szemben.
Tehát: (csoki, süti) $\succeq$ (vanília, süti) $\Rightarrow$ (csoki, fagyi) $\succeq$ (vanília, fagyi) és (csoki, nyár) $\succeq$ (vanília, nyár) $\Rightarrow$ (csoki, tél) $\succeq$ (vanília, tél). Ekkor az íz preferenciafüggetlen a többi kritériumtól.
...
\paragraph{Feladat:} Tegyük fel, hogy rendelkezünk jelenlegi értéken 1000\$-t érő,
az USA gazdaságnak erősen kitett részvényportfólióval.
Aminek a hozama várhatóan negatívan korrelál egy esetleges USA recesszióval.
A részvény portfóliót nem részletezzük, annyit tudunk, hogy historikusan magas értékeltségen van jelenleg.
Ennek köszönhetően alacsonyabb a várt egyéves hozamunk:
tegyük fel, hogy 6\%-ot várunk abban az esetben, ha USA növekedés lesz (1060\$-unk lesz 2026 végére).
USA recesszió esetén pedig 40\%-os esést várunk (600\$-unk lesz 2026 végére).
A feladat során további 100\$ készpénz befektetéséről hozunk döntést.
A célunk, hogy a rendelkezésünkre álló készpénzzel (100\$) olyan befektetést tegyünk,
ami USA recesszió esetén ellensúlyozza a részvényportfólión elért veszteségünket.

Erre a Polymarket-et fogjuk használni, ahol a \("\)US recession by end of 2026?\("\) játékban veszünk részt.
Ez egy olyan játék, amiben ha \("\)Igen\("\)-t választjuk, akkor az aktuális árfolyamnak megfelelően a teljesülés esetén
megnyerjük a \("\)Nem\("\) játékban résztvevők pénzét, veszteség esetén pedig elvesztünk mindent
(átadjuk a befektetett összeget a a \("\)Nem\("\) játékban résztvevőnek).

Tegyük fel, hogy az első alternatívánk az, hogy mind a 100\$-ból \("\)Igen\("\)-t veszünk.
Ekkor a Figure~\ref{fig:polyusarecession} alapján 251.08\$-t kapunk 2026 végén, amennyiben tényleg recesszió lesz.
Elveszítjük a 100\$-t, ha nem lesz recesszió.
A másik alternatíva, hogy nem veszünk részt ebben a játékban
és a pénzből a jelenlegi árfolyamon tovább növelnjük a részvény portfóliónk.

Összegezzük a két alternatíva egyes eseteit.
Amennyiben részt veszünk a játékban, akkor a recesszió esetén
a 600\$-os részvény portfólió érték mellé 251.08\$-t nyerünk a Polymarketen, összesen 851.08\$ várható pénzösszeg.
Ha nem lesz recesszió, akkor pedig 1060\$-unk lesz 2026 végére.
A másik alternatíva esetén az 1100 részvény befektetés várhatón recesszió esetén 660,
míg növekedés esetén 1166\$-unk lesz várhatóan 2026 végére.

\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.3]{PolymarketUSARecession2025-10-14.png}
    \caption{Polymarket USA recesszió 2026 végig (2025.10.14.-én.)}
    \label{fig:polyusarecession}
\end{figure}

A Polymarket-en a játékban résztvevők szerint 0.62 eséllyel nem lesz recesszió, míg 0.38 eséllyel recesszió lesz.
Mi ennél egy kicsit optimistábbak vagyunk és azt gondoljuk, hogy csak 0.2 eséllyel lesz recesszió
és 0.8 eséllyel kerüli el az USA a recessziót.
...
\multirow{2}{*}{\bf Cselekvési} &  \multicolumn{2}{c}{ \bf Események }\\
 {\bf lehetőségek} & $s_1=$ recesszió & $s_2=$ növekedés \\
\hline
  $a_1=$ polymarket \("\)Igen\("\) +100\$ & 851.08 & 1060.00 \\
  $a_2=$ növeljük a részvény portfóliónk +100\$ & 660.00 & 1166.00 \\
\end{tabular}
\end{center}
$P(s_1) = 0.2 , P(s_2) = 0.8$ szubjektív valószínűségi értékek (becslések).
...
\paragraph{Pesszimista, maximin kritérium:}  sorminimumok maximuma $\rightarrow a_1$
\paragraph{Optimista, maximax kritérium:}  sormaximumok maximuma $\rightarrow a_2$

\

De pénzérték alapján ezen enyhíteni szoktak $\rightarrow$ optimizmus együttható:
\emph{Hurwick-féle} $\alpha$, a döntéshozó $\alpha$ arányban optimista és $1-\alpha$ arányban pesszimista.
Legyen például $\alpha \gets 0.6$, ekkor a példánkban:
\begin{center}
$H_{a_1} \gets 0.6\cdot 1060.00 + 0.4\cdot 851.08 = 976.43$

$H_{a_2} \gets 0.6\cdot 1166.00 + 0.4\cdot 660.00 = 963.60$
...
Tehát a $P(s_2)$-nek a legnagyobb a valószínűsége, így az $a_2$-t választjuk.

\subsection{Döntés a rendelkezésre álló információ alapján}
Az alternatívákhoz várható pénzértéket számolunk a rendelkezésünkre álló információk alapján.
Rendelkezésünkre álló információ: a becsült várható pénzérték és a becsült valószínűségek.
Alternatívánként vesszük a természet állapotaihoz tartozó várható pénzértéket és azok valószínűségét.
Kiszámoljuk az állapotok becsült valószínűségekkel súlyozott összegét és kiválasztjuk közülük a maximálisat.

$VPRI(a_1) = 0.2\cdot 851.08 + 0.8\cdot 1060.00 = 1018.216$

$VPRI(a_2) = 0.2\cdot 660.00 + 0.8\cdot 1166.00 = 1064.80$

$VPRI = max\{VPRI(a_1), VPRI(a_2)\} = 1064.80$, az $a_2$-t választjuk.

\subsection{A tökéletes információ ára}
Ha élne egy jós, aki meg tudná mondani, hogy lesz-e recesszió
...
Első lépésben meg kell vizsgálni, hogyha az általunk ismert eseményeket állítja a jós,
akkor azokat biztosan bekövetkező eseményként vizsgálva mekkora a várható pénzérték.
Itt azért szerepel továbbra is egy valószínűségi súlyozás, mert így ad teljes eseményteret a két eset.
Ennek olyan jelentése van, hogy szerintünk ekkora valószínűséggel mondja a jós, hogy $s_{i}$ esemény következik be biztosan.
Ez megegyezik a korábbi valószínűséggel ugyanis a két esemény bekövetkezéséről továbbra is ugyanaz a véleményünk.
Amennyiben a jós bevonása ezen változtatna, akkor ez természetesen módosulhat.

Lehetséges előrejelzés:
\begin{itemize}
\item ha $s_{1}$ lesz a jós szerint $\rightarrow$ optimális cselekvés: $a_1$  és a $VP(a_1) = 0.20\cdot 851.08 = 170.216$
\item ha $s_{2}$ lesz a jós szerint $\rightarrow$ optimális cselekvés: $a_2$  és a $VP(a_2) = 0.80\cdot 1166.00 = 932.8$
\end{itemize}

A tökéletes információ várható pénzértéke ennek a két lehetőségnek az összege: 

$VPTI = 170.216 + 932.8 = 1103.01$

"Mennyit fizessünk a jósnak?" kérdésre a válasz: $VPTI - VPRI = 1103.01 - 1064.80 = 38.21$ (ez egy maximum érték, ennél többet nem ér az információ)

\subsection{Döntés nem tökéletes információ alapján}
\label{sec:NTI}

Mivel ténylegesen tökéletes információval rendelkező jósok nem léteznek,
ezért helyettük érdemesebb kockázatelemző, előrejelző céget keresni.
Ami a következő állításokat teheti:
\begin{itemize}
\item $z_{1}:$ azt állítják (előrejelzést tesznek), hogy recesszió lesz.
\item $z_{2}:$ azt állítják, hogy nem lesz recesszió. % $s_{2}$ következik be (általánosan: nem következik be a $z_{1}$-ben feltett esemény, itt speciálisan, ez az $s_{2}$)
\end{itemize}

Tegyük fel, hogy a cégről ismert \textbf{az előrejelzési pontossága}, korábbi évek előrejelzéseinek a pontossága alapján:
\begin{itemize}
\item $P(z_{1}|s_1) = 0.7$
\item $P(z_{2}|s_1) = 1- P(z_{1}|s_1) = 0.3$
\item $P(z_{2}|s_2) = 0.95$
\item $P(z_{1}|s_2) = 0.05$
\end{itemize}
Ezek a \textbf{beválási valószínűségek (likelihoodok)}.
Akár meg is egyezhetnének a $s_1$ és $s_2$-re, de általában a változás bekövetkezést nehezebb előre jelezni.
Ez az információ a feladatok többségében elérhető, ennek elérhetősége az előfeltétele a feladat végrehajtásának.
Az előző fejezetben például a jósnak 1 volt a találati értéke és 0 a tévesztési score-ja mindkét esetben.

Az ilyen feladatok esetében a $P(s_1) = 0.2 , P(s_2) = 0.8$ saját véleményen alapuló valószínűségi értékeket tekintjük
\textbf{a priori valószínűségek}nek.
...
\\
P(z_1)=\sum_{i}^{ } P(z_{1}|s_i) \cdot  P(s_i) =  0.7\cdot 0.2 + 0.05\cdot 0.8 &= 0.18 \\
P(z_2)=\sum_{i}^{ } P(z_{2}|s_i) \cdot  P(s_i) =  0.3\cdot 0.2 + 0.95\cdot 0.8 &= 0.82 \\
P(s_1|z_1) = \frac{0.14}{0.18} = 0.78,
P(s_2|z_1) = \frac{0.04}{0.18} &= 0.22 \\
P(s_1|z_2) = \frac{0.06}{0.82} = 0.07,
P(s_2|z_2) = \frac{0.76}{0.82} &= 0.93
\\
\end{split}
\end{equation}
...
$VP(a_1|z_1) = \sum_{i}^{ }vp(s_i,a_1)\cdot P(s_i|z_1) = 851.08\cdot 0.78 + 1060.00\cdot 0.22 = \textbf{897.04}$
 
$VP(a_2|z_1) = \sum_{i}vp(s_i,a_2)\cdot P(s_i|z_1) = 660.00\cdot 0.78 + 1166.00\cdot 0.22 = 771.32$

$VP(a_1|z_2) = \sum_{i}vp(s_i,a_1)\cdot P(s_i|z_2) = 851.08\cdot 0.07 + 1060.00\cdot 0.93 = 1045.38$

$VP(a_2|z_2) = \sum_{i}vp(s_i,a_2)\cdot P(s_i|z_2) = 660.00\cdot 0.07 + 1166.00\cdot 0.93 = \textbf{1130.58}$

\paragraph{A nem tökéletes információ várható pénzértéke: }

Ajánlatot kapunk az eddig vizsgált előrejelző cégtől: 10 dollárért előrejelzést adnak a recesszióra.
A kérdés megéri-e ennyiért elfogadni az ajánlatot.
Mennyit ér a nem tökéletes információ?

Lehetséges előrejelzés:
\begin{itemize}
\item $z_{1} \rightarrow$ optimális cselekvés: $a_1$  és a $VP(a_1) = VP(a_1|z_1) \cdot  P(z_1) = 897.04\cdot 0.18 = 161.47$
\item $z_{2} \rightarrow$ optimális cselekvés: $a_2$  és a $VP(a_2) = VP(a_2|z_2) \cdot  P(z_2) = 1130.58\cdot 0.82 = 927.08$
\end{itemize}

Nem teljes információhoz tartozó pénzérték: $VPNTI = \sum_{j} VP(a_j) = 161.47 + 927.08 = 1088.55$

A "Mennyit fizessünk legfeljebb az előrejelző cégnek?"
kérdésre a választ az alábbiak alapján tudjuk megkapni:
$VPNTI - VPRI = 1088.55 - 1064.80 = 23.75$.
Ami egy maximális érték arra, amennyiért megéri őket alkalmazni.
Mivel ez feletti ajánlat érkezett, ezért elutasítjuk.
...
\item az eseményekhez tartozó valószínűségeket be tudjuk helyettesíteni az egyes ágakra.
Egy esemény minél mélyebben van, annál több dologtól függ.
A lentebbi ágakon a felette lévőekkel alkot feltételes valószínűséget.
Pl: egy lentebbi ágon $P(s_1|z_1)$ van, míg felül $P(z_1)$ van (tehát ez utóbbi nem függ az $s_i$-től).
\item a levelekhez be tudjuk írni az egyes választások pénzértékét a táblázatból
(előrejelzés esetében már itt $d$ = 10 dollárral terhelt pénzértékeket).
\end{itemize}
...
\item döntési elágazás: kiválasztjuk a $max_{i\in K}\{gyerek_i\}$
és a nem maximális értékhez tartozó ágat levágjuk.
Ez lesz a csúcs várható pénzértéke.
\end{itemize}
...
\includegraphics[scale=0.7]{dectree06}
\\
\end{center}
...
Az előző fejezetben olyan speciális esetet néztünk, ahol mindkét alternatívára azonos esemény volt hatással. Ez most ennél általánosabb, az egyes alternatívákhoz független események és így független valószínűségek tartozhatnak.
...
\paragraph{Döntési szabály megalkotása  $\rightarrow$ Bernoulli elv: } A döntéshozó preferenciarendezését a lotterykhez tartozó várható pénzérték által kijelölt sorrend határozza meg. Tehát Bernoulli szerint $A^p$ elemeihez meghatározzuk a várható értékét, majd azt az alternatívát választjuk, amelyiknek legnagyobb a várható hasznossága: $u(a_{p_k})= \sum_{s_k \in S} P(s_k)\cdot v(c_{jk}) \rightarrow$ ezek közül azt az alternatívát választjuk, ahol maximális ez az érték. (Bernoulli elv röviden: $VPRI=max(u(a_{p_k}))$, várható pénzérték alapján döntünk).

Példa: $u(a_p)= 0.9\cdot 12 + 0.1\cdot 0 = 10.8$ és $u(a_q)= 0.9\cdot 1000000 + 0.1\cdot 0 = 900000$. Ezek közül az $a_q$ lotteryhez tartozó érték a maximális így ezt preferáljuk. (Ebben az esetben nem alkalmaztuk a $u$ értékelő függvényt, de ha az értékeket skálázzuk, és ezt tekintenénk értékelésnek, akkor is ezen eredményre jutnánk a döntés szempontjából.)

\subsection{Axiómarendszerek (Neumann-Morgenstern 1947)} 
...
\paragraph{Def:} Egy $u: A^p \rightarrow \mathbb{R}$ \textbf{mérhető} $\Leftrightarrow u$ tükrözi \textbf{(1)} az $A^p$-beli elemek sorrendjét és \textbf{(2)} az elemek különbségeinek a sorrendjét.

A mérhető $u$ létezésének vizsgálata nehézkes (itt most nem térünk ki rá).

Differenciaképzés: $\circ$.

\textbf{Mérhetőség feltétele:} $u$ mérhető, ha $\forall a,b,c,d \in A: a \circ b \succeq^* c \circ d$ $\Leftrightarrow$ $u(a)-u(b) \geq u(c)-u(d)$

\paragraph{Definíció: } Legyen $T=(X_1,...,X_m)$ tényezőhalmaz, $C \subseteq T$ gyenge differencia-független $C^*\subseteq T $-tól $\Leftrightarrow$ ha $\forall w_C,x_C,y_C,z_C \in X_C:$ valamely $w_{C^*}^0 \in X_{C^*}$ esetén teljesül minden más $w_{C^*} \in X_{C^*}$ -re, hogy: $(w_C, w_{C^*}^0) \circ (x_C, w_{C^*}^0) \succeq^* (y_C, w_{C^*}^0) \circ (z_C, w_{C^*}^0)$  $\Rightarrow$ $(w_C, w_{C^*}) \circ (x_C, w_{C^*}) \succeq^* (y_C, w_{C^*}) \circ (z_C, w_{C^*})$

\paragraph{Kvázi additivitási tétel} Tegyük fel, hogy
\begin{itemize}
\item létezik $u$ mérhető hasznosság függvény
\item a legkevésbé preferált $a^0: u(a^0) = 0$ és a legjobban preferált $a^1: u(a^1)=1$
\item hasonlóan $\forall i \in 1,..,n : $ a legkevésbé preferált $x_i^0: v_i(x_i^0) = 0$ és a legjobban preferált $x_i^1: v_i(x_i^1)=1$.
\end{itemize}
Ha $\forall i \in 1,..,n : X_i$ tényező gyenge-differenciafüggetlen a komplementerhalmazától, akkor \textbf{$u$ felírható kvázi additív alakban}.

%\textbf{Értékfüggetlenség $\Rightarrow$ Hasznosságfüggetlenség $\Rightarrow$ Preferenciafüggetlenség.}

\paragraph{Feltételek - Összegzés:}

\

Kölcsönös preferenciafüggetlenség $\Rightarrow$ additív forma

Kölcsönös hasznosságfüggetlenség  $\Rightarrow$ multiplikatív forma

Gyenge differenciafüggetlenség $\Rightarrow$ kvázi additív forma
